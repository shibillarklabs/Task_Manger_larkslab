...
const AllUsers = () => {
  ...
  return (
    ...
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      ...
      <Button className='bg-blue-600 hover:bg-blue-700 transition text-white font-semibold rounded-md px-4 py-2 text-sm sm:text-base w-full sm:w-auto'>+ Add User</Button>
    </div>
    <div className="mb-6 bg-gray-800 p-3 sm:p-4 rounded-md grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
      <Input type='text' placeholder='Filter by Name' className='px-3 sm:px-4 py-2 rounded-md bg-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base'  />
      <Input type='email' placeholder='Filter by Email' className='px-3 sm:px-4 py-2 rounded-md bg-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base' />
      <Select>
     ...
     {users.map((user: User, index: number)=>(
       <div key={user.id} className="grid grid-cols-5 gap-4 bg-gray-800 px-4 py-4 border-b border-gray-700 hover:bg-gray-700 transition">
        ...
        <div className="flex justify-end gap-4">
          <button className='text-yellow-400 hover:text-yellow-300 transition-transform hover:scale-110'><Pencil size={20}/></button>
        <button className='text-red-400 hover:text-red-300 transition-transform hover:scale-110' ><Trash2 size={20}/></button>
        </div>
      </div>
     ))}
  </div>
     ...
     <div  key={user.id} className="bg-gray-800 rounded-lg p-4 hover:bg-gray-700 transition">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-3">
          <button className='text-yellow-400 hover:text-yellow-300 transition-transform hover:scale-110 p-1' ><Pencil size={18}/></button>
          <button className='text-red-400 hover:text-red-300 transition-transform hover:scale-110 p-1'><Trash2 size={18}/></button>
        </div>
      </div>
